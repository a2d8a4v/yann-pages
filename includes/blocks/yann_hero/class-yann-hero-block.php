...
/**
 * Main Gutenberg Blocks for Yann_Hero Class
 *
 * @since 1.0.0
 */
class Yann_Hero {
	...
	public function register_blocks() {
		...
		register_block_type(
			$slug . '/hero', array(
				'editor_script' => $slug . '-hero-block-editor',
				'editor_style'  => $slug . '-hero-block-editor',
				'style'         => $slug . '-hero-block-frontend',
			)
		);
	}
	...
	public function editor_assets() {
		...
		// Fronted Styles.
		wp_register_style(
			$this->_slug . '-hero-block-frontend',
			$this->_url . '/src/blocks/hero/styles/yann-hero-block.css',
			array(),
			// $this->_version
			time()
		);
	...
	public function localization() {
		...
		wp_script_add_data( $this->_slug . '-hero-block-editor', 'data', $content );
	}
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'yann-hero', false, dirname( plugin_basename( $this->_dir ) ) . '/languages/' );
	}
}
