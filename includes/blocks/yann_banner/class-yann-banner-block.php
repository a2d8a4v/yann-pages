...
/**
 * Main Gutenberg Blocks for Yann_Banner Class
 *
 * @since 1.0.0
 */
class Yann_Banner {
	...
	public function body_class( $classes ) {
		...
		$classes[] = 'has-yann-banner-blocks';
	...
	public function localization() {
		...
		wp_script_add_data( $this->_slug . '-banner-block-editor', 'data', $content );
	}
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'yann-banner', false, dirname( plugin_basename( $this->_dir ) ) . '/languages/' );
	}

	public function YANN_NTNUSMIL_yann_banner_block_reutrn_background_media_url() {

		$media_id = $this->_media_id;
		$count = 0;
		$_size = "";
		foreach( wp_get_registered_image_subsizes() as $k => $v ) {
			if ($v["width"] >=$count) {
				$count = $v["width"];
				$_size = $k;
			}
		}
		$get = wp_get_attachment_image_src($media_id,$_size, true);

		return $get[0];
	}
}

Yann_Banner::register();
