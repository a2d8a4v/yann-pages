...
class YANN_Local_Avatars {
	...
	public function __construct() {
		$this->options        = (array) get_option( 'yann_local_avatars' );
		$this->avatar_ratings = array(
			'G'  => __( 'G &#8212; Suitable for all audiences', 'yann-local-avatars' ),
			'PG' => __( 'PG &#8212; Possibly offensive, usually for audiences 13 and above', 'yann-local-avatars' ),
			'R'  => __( 'R &#8212; Intended for adult audiences above 17', 'yann-local-avatars' ),
			'X'  => __( 'X &#8212; Even more mature than above', 'yann-local-avatars' ),
		);
	...
	public function add_hooks() {
		...
		add_action( 'admin_action_remove-yann-local-avatar', array( $this, 'action_remove_yann_local_avatar' ) );
		add_action( 'wp_ajax_assign_yann_local_avatar_media', array( $this, 'ajax_assign_yann_local_avatar_media' ) );
		add_action( 'wp_ajax_remove_yann_local_avatar', array( $this, 'action_remove_yann_local_avatar' ) );
	...
	public function get_avatar_data( $args, $id_or_email ) {
		...
		$yann_local_avatar_url = $this->get_yann_local_avatar_url( $id_or_email, $args['size'] );
		if ( $yann_local_avatar_url ) {
			$args['url'] = $yann_local_avatar_url;
		}

		// Local only mode
		if ( ! $yann_local_avatar_url && ! empty( $this->options['only'] ) ) {
	 ...
	 */
	public function get_yann_local_avatar_url( $id_or_email, $size ) {
		...
		$local_avatars = get_user_meta( $user_id, 'yann_local_avatar', true );
		...
		// check rating
		$avatar_rating = get_user_meta( $user_id, 'yann_local_avatar_rating', true );
		...
		if ( ! array_key_exists( $size, $local_avatars ) ) {
			...
			if ( apply_filters( 'yann_local_avatars_dynamic_resize', true ) ) :
				...
				update_user_meta( $user_id, 'yann_local_avatar', $local_avatars );
	...
	public function admin_init() {
		...
		if ( $old_ops = get_option( 'yann_local_avatars_caps' ) ) {
			if ( ! empty( $old_ops['yann_local_avatars_caps'] ) ) {
				update_option( 'yann_local_avatars', array( 'caps' => 1 ) );
			}

			delete_option( 'yann_local_avatar_caps' );
		}

		register_setting( 'discussion', 'yann_local_avatars', array( $this, 'sanitize_options' ) );
		add_settings_field(
			'yann-local-avatars-only',
			__( 'Local Avatars Only', 'yann-local-avatars' ),
			array( $this, 'avatar_settings_field' ),
			'discussion',
			'avatars',
			array(
				'key'  => 'only',
				'desc' => __( 'Only allow local avatars (still uses Gravatar for default avatars)', 'yann-local-avatars' ),
			)
		);
		add_settings_field(
			'yann-local-avatars-caps',
			__( 'Local Upload Permissions', 'yann-local-avatars' ),
			array( $this, 'avatar_settings_field' ),
			'discussion',
			'avatars',
			array(
				'key'  => 'caps',
				'desc' => __( 'Only allow users with file upload capabilities to upload local avatars (Authors and above)', 'yann-local-avatars' ),
			)
		);
	}
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		...
		$this->remove_nonce = wp_create_nonce( 'remove_yann_local_avatar_nonce' );

		$script_name_append = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '.dev' : '';
		wp_enqueue_script( 'yann-local-avatars', plugins_url( '', dirname( __FILE__ ) ) . '/js/admin/yann-local-avatars' . $script_name_append . '.js', array( 'jquery' ), false, true );
		wp_localize_script(
			'yann-local-avatars',
			'i10n_SimpleLocalAvatars',
			array(
				'user_id'          => $user_id,
				'insertMediaTitle' => __( 'Choose an Avatar', 'yann-local-avatars' ),
				'insertIntoPost'   => __( 'Set as avatar', 'yann-local-avatars' ),
				'deleteNonce'      => $this->remove_nonce,
				'mediaNonce'       => wp_create_nonce( 'assign_yann_local_avatar_nonce' ),
			)
		);
	}
	...
	public function avatar_settings_field( $args ) {
		...
		echo '
			<label for="yann-local-avatars-' . esc_attr( $args['key'] ) . '">
				<input type="checkbox" name="yann_local_avatars[' . esc_attr( $args['key'] ) . ']" id="yann-local-avatars-' . esc_attr( $args['key'] ) . '" value="1" ' . checked( $this->options[ $args['key'] ], 1, false ) . ' />
				' . esc_html( $args['desc'] ) . '
			</label>
		';
	}
	...
	?>
	<div id="yann-local-avatar-section">
		<h3><?php esc_html_e( 'Avatar', 'yann-local-avatars' ); ?></h3>

		<table class="form-table">
			<tr class="upload-avatar-row">
				<th scope="row"><label for="yann-local-avatar"><?php esc_html_e( 'Upload Avatar', 'yann-local-avatars' ); ?></label></th>
				<td style="width: 50px;" id="yann-local-avatar-photo">
			<?php
			add_filter( 'pre_option_avatar_rating', '__return_null' );     // ignore ratings here
			echo get_yann_local_avatar( $profileuser->ID );
			...
			if ( $upload_rights ) {
				do_action( 'yann_local_avatar_notices' );
				wp_nonce_field( 'yann_local_avatar_nonce', '_yann_local_avatar_nonce', false );
				$remove_url = add_query_arg(
					array(
						'action'   => 'remove-yann-local-avatar',
				...
				if ( ! current_user_can( 'upload_files' ) ) {
					?>
						<p style="display: inline-block; width: 26em;">
							...
							<input type="file" name="yann-local-avatar" id="yann-local-avatar" class="standard-text" />
							<span class="spinner" id="yann-local-avatar-spinner"></span>
						</p>
				<?php } ?>
						<p>
						<?php if ( current_user_can( 'upload_files' ) && did_action( 'wp_enqueue_media' ) ) : ?>
							<a href="#" class="button hide-if-no-js" id="yann-local-avatar-media"><?php esc_html_e( 'Choose from Media Library', 'yann-local-avatars' ); ?></a> &nbsp;
						<?php endif; ?>
							<a
								href="<?php echo esc_url( $remove_url ); ?>"
								class="button item-delete submitdelete deletion"
								id="yann-local-avatar-remove"
								<?php echo empty( $profileuser->yann_local_avatar ) ? ' style="display:none;"' : ''; ?>
							>
								<?php esc_html_e( 'Delete local avatar', 'yann-local-avatars' ); ?>
							</a>
						</p>
				<?php
			} else {
				if ( empty( $profileuser->yann_local_avatar ) ) {
					echo '<span class="description">' . esc_html__( 'No local avatar is set. Set up your avatar at Gravatar.com.', 'yann-local-avatars' ) . '</span>';
				} else {
					echo '<span class="description">' . esc_html__( 'You do not have media management permissions. To change your local avatar, contact the blog administrator.', 'yann-local-avatars' ) . '</span>';
				}
			}
			?>
				</td>
			...
			<tr class="ratings-row">
				...
				<td colspan="2">
					<fieldset id="yann-local-avatar-ratings" <?php disabled( empty( $profileuser->yann_local_avatar ) ); ?>>
						<legend class="screen-reader-text"><span><?php esc_html_e( 'Rating' ); ?></span></legend>
					<?php
					if ( empty( $profileuser->yann_local_avatar_rating ) || ! array_key_exists( $profileuser->yann_local_avatar_rating, $this->avatar_ratings ) ) {
						$profileuser->yann_local_avatar_rating = 'G';
					}

					foreach ( $this->avatar_ratings as $key => $rating ) :
						echo "\n\t<label><input type='radio' name='yann_local_avatar_rating' value='" . esc_attr( $key ) . "' " . checked( $profileuser->yann_local_avatar_rating, $key, false ) . "/> $rating</label><br />";
					endforeach;
					?>
						<p class="description"><?php esc_html_e( 'If the local avatar is inappropriate for this site, Gravatar will be attempted.', 'yann-local-avatars' ); ?></p>
					</fieldset></td>
			</tr>
		</table>
	</div>
	...
	public function assign_new_user_avatar( $url_or_media_id, $user_id ) {
		...
		update_user_meta( $user_id, 'yann_local_avatar', $meta_value );    // save user information (overwriting old)
	}
	...
	public function edit_user_profile_update( $user_id ) {
		// check nonces
		if ( empty( $_POST['_yann_local_avatar_nonce'] ) || ! wp_verify_nonce( $_POST['_yann_local_avatar_nonce'], 'yann_local_avatar_nonce' ) ) {
			return;
		}

		// check for uploaded files
		if ( ! empty( $_FILES['yann-local-avatar']['name'] ) ) :

			// need to be more secure since low privelege users can upload
			if ( false !== strpos( $_FILES['yann-local-avatar']['name'], '.php' ) ) {
				$this->avatar_upload_error = __( 'For security reasons, the extension ".php" cannot be in your file name.', 'yann-local-avatars' );
			...
			if ( is_wp_error( $avatar_id ) ) { // handle failures.
				$this->avatar_upload_error = '<strong>' . __( 'There was an error uploading the avatar:', 'yann-local-avatars' ) . '</strong> ' . esc_html( $avatar_id->get_error_message() );
				add_action( 'user_profile_update_errors', array( $this, 'user_profile_update_errors' ) );
				return;
			}
		...
		// Handle ratings
		if ( isset( $avatar_id ) || $avatar = get_user_meta( $user_id, 'yann_local_avatar', true ) ) {
			if ( empty( $_POST['yann_local_avatar_rating'] ) || ! array_key_exists( $_POST['yann_local_avatar_rating'], $this->avatar_ratings ) ) {
				$_POST['yann_local_avatar_rating'] = key( $this->avatar_ratings );
			}

			update_user_meta( $user_id, 'yann_local_avatar_rating', $_POST['yann_local_avatar_rating'] );
		}
	}
	...
	public function upload_size_limit( $bytes ) {
		return apply_filters( 'yann_local_avatars_upload_limit', $bytes );
	}

	/**
	 * Runs when a user clicks the Remove button for the avatar
	 */
	public function action_remove_yann_local_avatar() {
		if ( ! empty( $_GET['user_id'] ) && ! empty( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'remove_yann_local_avatar_nonce' ) ) {
			$user_id = (int) $_GET['user_id'];

			if ( ! current_user_can( 'edit_user', $user_id ) ) {
				wp_die( esc_html__( 'You do not have permission to edit this user.', 'yann-local-avatars' ) );
			}

			$this->avatar_delete( $user_id );    // delete old images if successful

			if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
				echo get_yann_local_avatar( $user_id );
			}
		}
	 ...
	 */
	public function ajax_assign_yann_local_avatar_media() {
		// check required information and permissions
		if ( empty( $_POST['user_id'] ) || empty( $_POST['media_id'] ) || ! current_user_can( 'upload_files' ) || ! current_user_can( 'edit_user', $_POST['user_id'] ) || empty( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'assign_yann_local_avatar_nonce' ) ) {
			die;
		}
	...
	public function avatar_delete( $user_id ) {
		$old_avatars = (array) get_user_meta( $user_id, 'yann_local_avatar', true );
		...
		delete_user_meta( $user_id, 'yann_local_avatar' );
		delete_user_meta( $user_id, 'yann_local_avatar_rating' );
	}
	...
	/**
	 * Registers the yann_local_avatar field in the REST API.
	 */
	...
	/**
	 * Returns the yann_local_avatar meta key for the given user.
	 *
	 * @param object $user User object
	 */
	public function get_avatar_rest( $user ) {
		$local_avatar = get_user_meta( $user['id'], 'yann_local_avatar', true );
